...
test('[utils -> resolvePathToTempDir]: should correctly resolve path to file in temp directory', (t) => {
...
test('[utils -> getUrlToHtmlFile]: should correctly generate url to file', (t) => {
...
test('[utils -> writeFile]: should write file in right place', async (t) => {
  const customTempDir = '../test/__mock__/'
...
test('[utils -> deleteFile]: should delete file in right place', async (t) => {
  const customTempDir = '../test/__mock__/'
  const fileName = `${nanoid()}.txt`
  const fileContent = 'information'
  const filePath = resolvePathToTempDir(fileName, customTempDir)
...
test('[utils -> readFile]: should read data from right file', async (t) => {
  const customTempDir = '../test/__mock__/'
  const fileName = `${nanoid()}.txt`
  const fileContent = 'information'
  const filePath = resolvePathToTempDir(fileName, customTempDir)
...
test('[utils -> megabitsToBytes]: should correctly transform megabits to bytes', async (t) => {
...
test('[utils -> getLibraryName]: should correctly extract library name', async (t) => {

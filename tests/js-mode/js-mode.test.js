...
test('estimoJsMode with RUNS option - should works properly', async (t) => {
  const chromeLocation = await findChrome()

  const lib1 = path.join(__dirname, '..', '__mock__', '19kb.js')
  const reports1 = await estimoJsMode([lib1], { executablePath: chromeLocation, runs: 3 })
  const reports2 = await estimoJsMode([lib1], { executablePath: chromeLocation, runs: 2 })

  t.is(reports1.length, 1)
  t.is(reports1[0].name, '19kb.js')

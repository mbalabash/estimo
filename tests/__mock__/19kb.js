void (function () {
  ...
  smoke.build = function (text, params) {
    ...
    var buttons = (dialog.buttons = document.createElement('div'))
    buttons.className = css_prefix + '-dialog-buttons'

    // Add a title element to the modal.
    if (
      typeof title.text != 'undefined' ||
      typeof title.close != 'undefined' ||
      typeof title.close_reference != 'undefined'
    ) {
      // Create the actual title element.
      var title_element = (dialog.title_element = document.createElement('div'))
      ...
      if (typeof title.text != 'undefined') {
        var title_text = (title_element.text = document.createElement('div'))
        title_text.className = css_prefix + '-dialog-title-text'
    ...
    var text_div = (dialog.text = document.createElement('div'))
    ...
    if (params.type == 'prompt') {
      var prompt = (modal.dialog.prompt = document.createElement('div'))
    ...
    if (dialog.title_element && dialog.title_element.title_close) {
      dialog.title_element.title_close.addEventListener(
        dialog.params.point_event,
        dialog.cancel ? dialog.cancel : dialog.ok
      )
      dialog.title_element.title_close.smoke_pure_modal = modal
    }

    if (typeof modal.dialog.prompt != 'undefined' && autofocus != false)
      modal.dialog.prompt.input.focus()
    ...
    for (var value in params) {
      if (smoke.extension[value])
        smoke.extension[value](params[value], modal, text, callback, params)
    }
    return modal
  }

  smoke.extend = function (value, func) {
    smoke.extension[value] = func
  }

  smoke.finishbuilding_alert = function (modal) {
    var dialog = modal.dialog
    dialog.callback_ok = function () {
      dialog.params.callback()
    }
    var ok_function_wrapper = (dialog.ok = function (evt) {
      ok_function(evt, modal, { allow_esc: true })
    })
  ...
  smoke.finishbuilding_confirm = function (modal) {
    var dialog = modal.dialog
    dialog.callback_ok = function () {
      dialog.params.callback(true)
    }
    dialog.callback_cancel = function () {
      dialog.params.callback(false)
    }
    var ok_function_wrapper = (dialog.ok = function (evt) {
      ok_function(evt, modal)
    })
    var cancel_function_wrapper = (dialog.cancel = function (evt) {
      cancel_function(evt, modal)
    })
    smoke.add_global_listener(dialog, 'keyup', ok_function_wrapper)
  ...
  smoke.finishbuilding_prompt = function (modal) {
    var dialog = modal.dialog
    dialog.callback_ok = function () {
      dialog.params.callback(dialog.prompt.input.value)
    }
    dialog.callback_cancel = function () {
      dialog.params.callback(false)
    }
  ...
  function ok_function(evt, modal, options) {
    ...
    if (
      ...
      (evt.keyCode != 13 || (options.allow_esc && evt.keyCode != 27))
    )
      return

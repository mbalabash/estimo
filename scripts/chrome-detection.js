...
let puppeteerChromeRevision

try {
  // eslint-disable-next-line
  puppeteerChromeRevision = require(path.join(
...
const MAX_CHROME_VERSION = parseInt(
  process.env.MAX_CHROME_VERSION ||
    process.env.NPM_CONFIG_MAX_CHROME_VERSION ||
    process.env.npm_config_max_chrome_version ||
    process.env.NPM_PACKAGE_CONFIG_MAX_CHROME_VERSION ||
    process.env.npm_package_config_max_chrome_version ||
    '80'
)
const LATEST_STABLE_CHROME_REVISION = puppeteerChromeRevision || '722234'
...
async function isSuitableVersion(executablePath) {
  ...
  if (match && match[2]) {
    ...
    return version >= MIN_CHROME_VERSION && version <= MAX_CHROME_VERSION
  }
  return false
}
...
module.exports.findLocalChrome = findLocalChrome

module.exports.findChrome = async function findChrome(distPath = chromeConfigPath) {
  let executablePath
  try {
    // eslint-disable-next-line
    const chromeConfig = require(distPath)
    if (typeof chromeConfig.executablePath === 'string' && chromeConfig.executablePath.length) {
  ...
  if (typeof executablePath === 'string' && executablePath.length > 0) {
    if (await isSuitableVersion(executablePath)) {
      await writeFile(distPath, JSON.stringify({ executablePath }))
  ...
  await writeFile(distPath, JSON.stringify({ executablePath: revisionInfo.executablePath }))
